...
// * ed25519_keccak: `R (32-byte) || s (32-bytes)` - `64 bytes` using keccak as hash function
// * schnorr_1: `r (32-bytes) || s (32-bytes)` - `64 bytes`
//   (schnorr signature implemented by Zilliqa where both `r` and `s` are
...
//   (schnorr signature w/ Poseidon hash function implemented by O(1) Labs where both `r` and `s` are scalars
...
const (
	...
	Ed25519_Keccak  SignatureType = "ed25519_keccak"
